...
static int __init s5pv210_cpufreq_init(void)
{
	...
	unsigned int regval;
	/* Check chip ID*/
	regval = readl(S5P_VA_CHIPID);
	if (regval != 0x43110231) {
		pr_err("ERROR: product ID is not match for 1.2GHz work\n");
		goto error;
	}

	/* Read ASV value */
	regval = readl(S5P_VA_CHIPID + 0x10) & 0x1f;
	if ((regval >= 11) && (regval <= 20))
		dvs_conf = dvs_conf_asv1;
	else if ((regval >= 21) && (regval <= 30))
		dvs_conf = dvs_conf_asv2;
	else if ((regval >= 1) && (regval <= 10))
		dvs_conf = dvs_conf_asv0;
	else {
		pr_err("ERROR: wrong asv value\n");
		goto error;
	}
